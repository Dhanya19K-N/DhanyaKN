...
window.onload = function(){
   ...
   document.getElementById("homeBtn").onclick = function(){
        box.innerHTML = `
        <h2>Real-Time Support For Every Woman</h2>
        ...
        if (navigator.geolocation) {
            navigator.geolocation.getCurrentPosition(
                (pos) => {
                   ...
                   sss <input type="tel" id="smsNumber" placeholder="Enter phone number" style="width:80%;padding:6px;" /><br><br>
  ...
  document.getElementById("chatBtn").onclick = function(){
        ...
        document.getElementById("sendChatBtn").onclick = function(){
            ...
            // Clear input
            chatInput.value = "";// Create message status ✓ mark
    ...
    // Contact Access Button Functionality
    document.getElementById("contBtn").onclick = function(){
        try {
            // Check if contact picker is supported
            if(!("contacts" in navigator && "select" in navigator.contacts)) {
                alert("Contact access is not supported on this device/browser.")
                return;
            }
            // Ask user what information you want to fetch
             const props = ["name","num","email"];
             // Request access
             const cantacts = await
             navigator.contacts.select(props,
                { multiple: false});
                if(contacts.length>0) {
                    let contact = contacts[0];
                    alert(
                        "Name:" + (contact.name ? cantacts.name : "No Name") + "\nPhone:" +
                        (contact.tel ? contact.tel : "No Phone") + "\nEmail: "  +
                    (contact.email ? contact.email : "No Email")
                        );
                }
            } catch (error) {
                console.log(error);
                alert("Permission denied or error accessing contacts.");
            }
